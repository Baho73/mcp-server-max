...
| Инструмент | Описание |
|------------|----------|
| `send_message` | Отправить сообщение (текст, markdown, HTML) с поддержкой ответов |
...
| `get_pinned_message` | Получить закреплённое сообщение |
| `leave_chat` | Покинуть чат |
| `get_bot_info` | Информация о боте |
| `send_typing` | Отправить индикатор набора |
| `get_updates` | Получить последние события (long polling) |
